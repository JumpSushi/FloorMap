...
export class IndoorGeocoder {
  ...
  public getAutocompleteResults(
    ...
    // Otherwise, default to the top 3 results.
  ...
  private getCutoffIndex(results: SearchResult[], topScore: number): number {
    for (let i = 1; i < results.length; i++) {
      const scoreDifference = (topScore - results[i].score) / topScore;
      if (scoreDifference > this.cutoffThreshold) {
        return i;
      }
    }
    return -1; // No cutoff point found
  }
}
